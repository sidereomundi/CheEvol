...
c     declarations
      ...
      real pSFR,superf,Pwind,Pinfall,delay,var
...
!      real ampli,pi,sinsfr,azi,period !spiral
...
!!!     Modification for implement output by Ivan
!!      real ivan(36,20,300),ti(300)
      real vdens
!!      integer ri,  fi
!!      character*3 namei(300)
      
      pi=3.141592654
      
      var=1
      
      ss=0          ! Index for SNIa bins
      Norm=0.0      ! Normalization of the IMF (real, supposed to be 1, slightly less!)
      snian=0.0     ! Number of SNIa in the mass steps
      hottime=0     ! Possible to insert a delay between the death of the stars and
!                  the chemical enrichment of the ISM

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      write (*,*) 'IMF? 1=Scalo, 2=Kroupa  3=Salpeter'
      !read(*,*) IMF
      IMF=1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!      write (*,*)'Which tau stars normal (1)or dwarf model(2)?'
!      read(*,*) tautype
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!      write (*,*)'Which yields for low itermediate mass stars?'
!      write (*,*) 'Van den Hoek & Groenewegen (1) or Karakas (2)'
!      read(*,*) lowmassive
...
!
 ...
 104  format (6e13.5)
     ...
     $           W(20,n,z),W(21,n,z), W(22,n,z),W(23,n,z)
            ...
            write (*,102) massa(n),W(1,n,z)+W(2,n,z)+W(3,n,z)+W(4,n,z)+
     $           W(5,n,z)+W(6,n,z)+ W(7,n,z)+W(8,n,z)+W(9,n,z)+
     $           W(10,n,z)+W(11,n,z)+W(12,n,z)+W(13,n,z)+W(14,n,z)+ 
     $           W(15,n,z)+W(16,n,z)+W(17,n,z)+W(18,n,z)+W(19,n,z)+
     $           W(20,n,z)+W(21,n,z)+ W(22,n,z)+W(23,n,z)
...
!
!      z=11 !z= 0.0001
!      OPEN(17,file='./DATI/'//'Siess0001'//siess,STATUS='old')
!      read(17,*)
!      read(17,*)
!      write(*,*) 'Siess0001'
!      do n=1,7
!         read (17,100) massas(n),W(1,n,z),W(2,n,z),W(3,n,z),W(4,n,z),
!     $        W(5,n,z),W(6,n,z), W(7,n,z),W(8,n,z),W(9,n,z),W(10,n,z),
!     $        W(11,n,z),W(12,n,z),W(13,n,z),W(14,n,z)
!         write (*,100) massas(n),W(1,n,z),W(2,n,z),W(3,n,z),W(4,n,z),
!     $        W(5,n,z),W(6,n,z), W(7,n,z),W(8,n,z),W(9,n,z),W(10,n,z),
!     $        W(11,n,z),W(12,n,z),W(13,n,z),W(14,n,z)
!
!      enddo
...
!
!      z=12 !z= 0.001
!      OPEN(17,file='./DATI/'//'Siess001'//siess,STATUS='old')
!      read(17,*)
!      read(17,*)
!      write(*,*) 'Siess001'
!      do n=1,7
...
!
!      z=13 !z= 0.004
!      OPEN(17,file='./DATI/'//'Siess004'//siess,STATUS='old')
!      read(17,*)
!      read(17,*)
!      write(*,*) 'Siess004'
!      do n=1,7
...
!
!      z=14 !z= 0.008
!      OPEN(17,file='./DATI/'//'Siess008'//siess,STATUS='old')
!      read(17,*)
!      read(17,*)
!      write(*,*) 'Siess008'
!      do n=1,7
...
!
!      z=15 !z= 0.02
!      OPEN(17,file='./DATI/'//'Siess02'//siess,STATUS='old')
!      read(17,*)
!      read(17,*)
!      write(*,*) 'Siess02'
!      do n=1,7
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!
!!!!!!Subroutine per leggere i valori del Ba and La "s" process
!!!!!!
      call leggili
      call leggila
      call leggirb
      ...
      write (*,*) 'Which name do you like to give to the output file?'
      read(*,*) nameout
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!   Se s'intende prendere i valori per bin di masse originali
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     $              multi1(jj)*0.05 ! weight of this configuration!
...
!!!!  Parameters 
!!!!!!!
!!!!!!    Model as the THIN model (so no 2 infall T=12Gyr)
!!!!!!

      p=1                       ! Just one run for an homogenous model
      superf=20000.             ! Surface of the volume in pc^2 
      step=1                    ! Timestep in Myr usually set in 1Myr 
      psfr=1.2                  ! superf/1000.*0.1
      endoftime=13700           !Valore conclusivo per il tempo
!      endoftime=2000 !thick disc
...
!      RD=3.5
!      SD=491.
!      SigmaR=SD/EXP(Raggio/RD)

      Sigmar=65
      
      tauR=(Raggio*1.03-1.27)*1000 
      SigmaH=0.
      TauH=0.
      T1=800.
      ...
      Sigmasun=65.
      Tsolar=7000.
      if (Raggio<3) TauR=0.8
      
!      sigmar=6.5 ! Thick disc
!      TauR =100 ! Thick disc
...
!!!   
         ...
         do  !!! UPDATE FOR ALL THE TIME THE AMOUNT OF TOTAL MASS AND GAS
            ...
            mingas=sigmar/10.  ! (bis/tris)
            

            all(t3)=superf*mingas
!     $           +(sigmah-mingas)*((superf)*(1./(1.-exp(-TAUH/T1))))*
!     $           (1.-1./exp(real(t3)/T1))

            
            allS(t3)=all(t3)

            if (real(t3).gt.tauh) then

               all(t3)=all(t3)
     $              +(SigmaR-mingas)*((superf)*
     $              (1./(1.-exp(-(endoftime-tauh)/tauR))))*
     $              (1.-1./exp(real(t3-tauh)/taur))

               allS(t3)=allS(t3)
     $              +(sigmasun-mingas)*((superf)*
     $              (1./(1.-exp(-(endoftime-tauh)/(tauR)))))*
     $              (1.-1./exp(real(t3-tauh)/(Tsolar)))
...
!         call vardens(azi,raggio,real(t),ivan,ti,vdens)
!         vdens=1.+(2*(vdens-1.))
!         if (vdens.lt.0.) vdens=1.e-4
         ...
         if (all(t).gt.0.) then
            SFR= psfr*(gas(t)/superf*vdens)**1.5
     $        *        (SigmaR*Superf/All(t))**0.5
     $        *        (all(t)/alls(t)) 
     $        *        20/(sigmar)**0.5 !!!! conversione in unita' di questo programma!!!!!
...
!!!   
         write(*,122) real(t), SFR,  all(t+1) ,gas(t), 
     $        stars(t),norm
